...
The implementation emits clients sorted ascending and renders monetary values
with at most four decimal places and at least one decimal digit. The golden test
harness compares stdout directly with the expected file, ignoring only a trailing
newline difference, so expected files must match the engine's exact rendered CSV.
...
| 02 | withdrawal-insufficient-funds | A withdrawal larger than `available` is skipped, logged to `stderr`, and leaves `total` unchanged. | Balance stays `5.0`. |
...
| 09 | cross-client-dispute-mismatch-ignored | Dispute whose `client` doesn't own the referenced `tx` is ignored (README Assumption #11). | Both clients unchanged at `5.0`. |
| 10 | dispute-withdrawal-ignored | Only deposits are disputable; a dispute referencing a withdrawal is ignored (README Assumption #10). | Disputing the `4.0` withdrawal is a no-op: `available 6`, `held 0`, `total 6`. |
| 11 | redispute-after-resolve | A tx may be disputed again **after** it was resolved (README Assumption #12). | Second dispute re-holds: `available 0`, `held 10`. |
| 12 | no-dispute-after-chargeback | A charged-back tx is final and cannot be re-disputed (README Assumption #12). | Second dispute ignored; state identical to case 05. |
| 13 | locked-account-rejects-new-tx | A locked account rejects later deposits and withdrawals (README Assumption #13). | Post-chargeback deposit/withdrawal ignored; `total 0`, `locked true`. |
| 14 | negative-available-during-dispute | Disputing a spent deposit may drive `available` negative — the bank holds the full amount (README Assumption #14). | `available -5`, `held 10`, `total 5`. |
| 15 | duplicate-tx-id-ignored | A repeated `tx` ID is ignored on its second occurrence (README Assumption #16). | Only the first `5.0` deposit counts. |
...
| 17 | unknown-type-skipped | An unknown transaction `type` is skipped, run continues (README Assumption #17). | Only the two valid deposits count: `7.0`. |
